...
if __name__=='__main__':
    ...
    #if random_seed=='Y':embedding=calculate_umap(tensor_list,n_neighbors=n_neighbors,min_dist=min_dist,metric=metric,random_num=False)#returns points in 2-D
    #else: embedding=calculate_umap(tensor_list,n_neighbors=n_neighbors,min_dist=min_dist,metric=metric,random_num=True)
    nametotypedict={}#keys: names of proteins, values: the type on the chosen categorization
    seen = set()
